<main>
    <?php
        if($type == "teacher"){
            $updatedid = $_GET['id'];
            $select_sql = $connec->query('SELECT * FROM homework WHERE id = ?', $updatedid)->fetchArray(); 
    ?>
    <section id="lesson">
        <div class="">
           <div class="login-form roundborder">
                <div class="login-logo">
                    <h2> Edit Homework's Data </h2>
                </div>
                <?php
                    ...
                    elseif(isset($_GET['editDerror']) && $_GET['editDerror'] == 2){
                        echo "<div style='color:red'>Homework Title Exist </div>";
                    }
                ?>
                <form  action="homeworkAfterEditData" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group row col-12 nopadd">
                        ...
                        <div class="col-12 col-md-9 nopadd leftmargin"><input type="text" class="form-control" placeholder="Homework Title" name="title" value="<?=$select_sql['title']?>" required></div>
                    </div>
                    <div class="form-group row col-12 nopadd">
