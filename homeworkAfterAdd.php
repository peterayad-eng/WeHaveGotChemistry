<?php
	...
	if(!isset($_SESSION['user']) || $_SESSION['user'] == ""){
        ...
        if($repeatflag == 0){
            if(array_key_exists('url', $_FILES)){
                if ($_FILES['url']['error'] === UPLOAD_ERR_OK) {
                    if($fileSize>SIZE){
                        ...
                        header("location: homeworksAdd.php?editLerror=2&id=$Addedid");
                    }
                        ...
                        $added_sql = "INSERT INTO homework (id, title, par, url, level) VALUES ('$Addedid', '$title', '$par', '$docName', '$level');";
                        if ($conn->query($added_sql) === TRUE) {
                            $connec->disconnect($conn);
                            header("location: homeworks.php?adderror=0");
                        } else {
                            $connec->disconnect($conn);
                            header("location: homeworksAdd.php?adderror=1&id=$Addedid");
                        }
                    }else{
                        $connec->disconnect($conn);
                        header("location: homeworksAdd.php?editLerror=3&id=$Addedid");
                    }
            ...
            $connec->disconnect($conn);
            header("location: homeworksAdd.php?adderror=2&id=$Addedid");
        }
?>